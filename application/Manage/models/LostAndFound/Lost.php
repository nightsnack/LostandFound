...
class Lost extends CI_Model
{
    ...
    {
        $this->_db->select('a.item_id as 物品编号,a.item_name as 物品名称 ,b.name as 物品类型,a.student_id as 发布人帐号 ,a.release_name as 发布人姓名,a.tel as 发布人电话,a.position as 捡到地点,a.time as 捡到时间,d.name as 找回状态');
        $this->_db->from("$this->table as a");
        $this->_db->join('item_type as b', "a.type_id = b.type_id",'inner');
        $this->_db->join('retrieve_status as d', "a.retrieve_id = d.retrieve_id",'inner');
