...
{
	/* 所有字段名称 */
	private $key = array("open_id", "student_id", "zhxy_psw", "jwxt_psw", "tsg_psw", "aolan_psw");
	/* 主键名 */
	private $primary_key = "open_id";
	/* 表名 */
	private $table = "student_password";
	/* 置空值 */
	...
	{
		$arr = $this->db->get_where($this->table, array($this->primary_key => $open_id))->result();
		if (sizeof($arr) == 0) {
			return null;
		} else {
			$obj = $arr[0];
