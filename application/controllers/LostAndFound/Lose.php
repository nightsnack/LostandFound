...
class Lose extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('LostAndFound/Lost');
    ...
    public function showItems($item_type,$current_page = 1)
    {
        ...
        $config['base_url'] = site_url("LostAndFound/Lose/showItems/$item_type");
        ...
        $this->load->view('templates/header');
        $this->load->view('LostAndFound/LostFeedback',$pass);
        $this->load->view('templates/footer');
    }
    ...
    public function showDetail($item_id)
    {
        ...
        {
            $front['display'] = 'block';
            $front['action'] = site_url('LostAndFound/Lose/updateLose');
        ...
        $this->load->view('templates/header');
        $this->load->view('LostAndFound/LostDetails',$front);
        $this->load->view('templates/footer');
    }
    ...
    public function newLose()
    {
        ...
        $this->load->view('templates/header');
        $this->load->view('LostAndFound/NewLost',$res);
        $this->load->view('templates/footer');
    }
    ...
    public function showUpdateLose($item_id)
    {
        ...
        $front=$item_info['0'];
        $this->load->view('templates/header');
        $this->load->view('LostAndFound/UpdateLost',$front);
        $this->load->view('templates/footer');
    }
    ...
    public function insertNewLose()
    {
        ...
        if  ($res['status'] === 1)
        {
            redirect("LostAndFound/Lose/showDetail/{$res['id']}");
        }
        else {
            redirect("LostAndFound/Lose/newLose");
        }
    }
    /**
     * 用来接更新物品详情的post
    ...
    public function updateLose()
    {
        ...
        if ($res==1)
        {
            $url = site_url("LostAndFound/Lose/showDetail/{$post_data['item_id']}");
